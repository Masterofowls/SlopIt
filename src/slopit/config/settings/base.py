...
# ─── Paths ───────────────────────────────────────────────────────────────────
BASE_DIR = Path(__file__).resolve().parent.parent.parent  # → src/slopit
...
# Unfold MUST come before django.contrib.admin to override templates.
INSTALLED_APPS: list[str] = [
    ...
    # MFA (TOTP + WebAuthn passkeys) — needs fido2>=1.1.
    "allauth.mfa",
    "allauth.headless",
    "django_rq",
    "django_extensions",
...
# ─── Database ────────────────────────────────────────────────────────────────
# Requires PostgreSQL — uses ArrayField and SearchVectorField.
DATABASES = {
    "default": env.db_url(
        "DATABASE_URL",
...
# ─── AllAuth (OAuth-only, no password login) ──────────────────────────────────
...
# ─── AllAuth MFA (TOTP + WebAuthn passkeys) ──────────────────────────────────
MFA_TOTP_ISSUER = "SlopIt"
MFA_TOTP_PERIOD = 30
MFA_TOTP_DIGITS = 6
# Overridden in dev.py to True so localhost is a valid RP origin.
MFA_WEBAUTHN_ALLOW_INSECURE_ORIGIN = False

# ─── AllAuth headless (JSON responses for React SPA) ─────────────────────────
HEADLESS_ONLY = False  # keep HTML admin/allauth pages working
HEADLESS_TOKEN_STRATEGY = "allauth.headless.tokens.sessions"  # use session cookies
...
FEED_MAX_LIFETIME_HOURS = env.int("FEED_MAX_LIFETIME_HOURS", default=48)
FEED_PAGE_SIZE = env.int("FEED_PAGE_SIZE", default=25)
FEED_BUCKET_COUNT = 256  # L2: anti-clustering buckets (see docs/ALGORITHM.md)

# ─── Background queues (django-rq) ──────────────────────────────────────────
# When REDIS_URL is set, jobs run asynchronously in Redis-backed queues.
# When REDIS_URL is not set (e.g. simple prod deploy), IS_ASYNC=False makes
# every .enqueue() call execute the job inline — no Redis required.
    ...
    "SIDEBAR": {
        ...
        "navigation": [
            ...
            },
            {
                ...
                "separator": True,
                "items": [
                    {"title": "Reports", "icon": "flag", "link": "/admin/moderation/report/"},
                    {"title": "Bans", "icon": "block", "link": "/admin/moderation/ban/"},
                ],
            },
            {
                "title": "Pages & Config",
                "separator": True,
                "items": [
                    {"title": "Static Pages", "icon": "web", "link": "/admin/pages/staticpage/"},
                    {
                        "title": "System Flags",
                        "icon": "settings",
                        "link": "/admin/pages/systemflag/",
                    },
                ],
            },
        ],
