...
Public API:
    ...
    initialize_system_feed()    -> int          # one-shot bootstrap
...
def upsert_post_feed_meta(post: Post) -> PostFeedMeta:
    ...
    """
    # Lazy import to avoid circular dependency (L1 ↔ L2).
    ...
    # Update the full-text search vector (SearchVector requires a separate UPDATE).
