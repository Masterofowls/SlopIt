...
def _build_ordered_ids(
    ...
    # Within-bucket ordering: rotation_offset XOR low 32 bits of seed.
...
def force_new_snapshot(user: AbstractBaseUser) -> FeedSnapshot:
    ...
    # Cryptographically random seed; 63 bits to stay within signed bigint range.
    seed = secrets.randbelow(2**63)

    # ── Build filtered eligible pool ─────────────────────────────────────────
    ...
    if prefs.filter_words:
        # Build an OR query for each filter word and exclude matching posts.
