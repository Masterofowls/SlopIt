...
class User(AbstractUser):
    # Override email to enforce uniqueness across the platform.
...
class Profile(models.Model):
    ...
    # Per-user feed lifetime override (10-48 h; must respect system bounds).
...
class Passkey(models.Model):
    ...
    # credential_id is a base64url-encoded byte string; max 1024 chars per spec.
    ...
    sign_count = models.PositiveBigIntegerField(default=0)  # replay attack counter
    aaguid = models.CharField(max_length=36, blank=True)  # authenticator identifier
    name = models.CharField(max_length=100, blank=True)  # user-given nickname
    created_at = models.DateTimeField(auto_now_add=True)
...
class Passphrase(models.Model):
    ...
    phrase_hash = models.CharField(max_length=256)  # bcrypt hash
    created_at = models.DateTimeField(auto_now_add=True)
