...
const ProfilePage = () => {
  ...
  if (!isLoaded) return <div className="pp-page"><FrogBackground /><p className="pp-status">// LOADING…</p></div>;
      ...
      <div className="pp-container">
        <div className="pp-card pp-header-card">
          <div className="pp-avatar-wrap">
            {avatarUrl ? <img src={avatarUrl} alt="avatar" className="pp-avatar" /> : <div className="pp-avatar-placeholder">{displayName[0].toUpperCase()}</div>}
            <div className="pp-avatar-glow" />
          </div>
          <div className="pp-identity">
            <h1 className="pp-displayname">{displayName}</h1>
            {clean(profile?.username) && <p className="pp-username">@{clean(profile.username)}</p>}
            {profile?.bio && <p className="pp-bio">{profile.bio}</p>}
          </div>
          <div className="pp-stats">
            <div className="pp-stat"><span className="pp-stat-val">{posts.length}</span><span className="pp-stat-label">posts</span></div>
          </div>
          <div className="pp-clerk-btn"><UserButton afterSignOutUrl="/" /></div>
          <button className="pp-edit-toggle" onClick={editing ? cancelEdit : openEdit}>{editing ? "✕ CLOSE" : "✎ EDIT"}</button>
        </div>
          ...
          <div className="pp-card pp-edit-panel">
            ...
            <div className="pp-edit-avatar-row">
              <div className="pp-edit-avatar-preview">
                {(avatarPreview || avatarUrl) ? <img src={avatarPreview || avatarUrl} alt="avatar" /> : <div className="pp-edit-avatar-placeholder">{displayName[0].toUpperCase()}</div>}
              </div>
              <label className="pp-edit-upload-btn">CHOOSE AVATAR<input type="file" accept="image/*" onChange={onAvatarChange} style={{ display: "none" }} /></label>
            </div>
            <div className="pp-edit-field"><label className="pp-edit-label">DISPLAY NAME</label><input className="pp-edit-input" maxLength={100} placeholder="Enter your display name…" value={editName} onChange={e => setEditName(e.target.value)} /></div>
            <div className="pp-edit-field"><label className="pp-edit-label">BIO</label><textarea className="pp-edit-input pp-edit-textarea" maxLength={500} placeholder="Tell the world…" value={editBio} onChange={e => setEditBio(e.target.value)} rows={3} /></div>
            <div className="pp-edit-actions">
              <button className="pp-edit-btn pp-edit-btn--primary" onClick={saveProfile} disabled={saving}>{saving ? "SAVING…" : "SAVE"}</button>
              <button className="pp-edit-btn pp-edit-btn--cancel" onClick={cancelEdit} disabled={saving}>CANCEL</button>
            </div>
          </div>
        )}
        ...
        <div className="pp-posts-grid">
          {posts.map(p => (
            <div key={p.id} className="pp-post-card" onClick={() => p.slug && navigate(`/post/${p.slug}`)} style={{ cursor: p.slug ? "pointer" : "default" }}>
              {p.media?.[0]?.file && <div className="pp-post-thumb-wrap"><img src={p.media[0].file} alt={p.title} className="pp-post-thumb" /></div>}
              <div className="pp-post-info"><span className={`pp-post-kind pp-kind--${p.kind}`}>{p.kind}</span><p className="pp-post-title">{p.title}</p></div>
            </div>
          ))}
        </div>
      </div>
