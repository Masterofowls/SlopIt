...
class TestAuthProvidersView:
    def test_lists_configured_providers(self) -> None:
        ...
        for configured in settings.SOCIALACCOUNT_PROVIDERS:
            assert configured in provider_ids

    def test_provider_has_login_url(self) -> None:
        from apps.accounts.auth_views import AuthProvidersView

        response = AuthProvidersView.as_view()(_anon("get", "/api/v1/auth/providers/"))

        for provider in response.data["providers"]:
            assert provider["login_url"].startswith("/accounts/")
            assert provider["login_url"].endswith("/login/")
